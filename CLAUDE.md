...
- Treat as untrusted input: downloaded files, README instructions, notebooks, issue text, and —
  ...
  matters here, so never `eval`/`exec` config content; load YAML with `yaml.safe_load`, never
...
`.circleci/config.yml` and `.ci-pipelines/` (an Azure Pipelines release job and its dockerfile)
are 2021-era leftovers. They reference a repo-root `requirements.txt` that no longer exists and
do not gate PRs. GitHub Actions is the only live CI.
...
`.release/changeVersionNumbers.sh X.Y.Z` (run from inside `.release/`) bumps the version. It
replaces the first `N.N.N` on each line of `docs/source/conf.py`, `gcpy/_version.py`,
`gcpy/benchmark/run_benchmark.py`, and the two `gcpy/benchmark/modules/run_1yr_*_benchmark.py`
drivers; sets `MAJOR`/`MINOR`/`MICRO` in `setup.py`; and rewrites the `[Unreleased]` heading
in `CHANGELOG.md` to `[X.Y.Z] - <today>`. Things to check afterwards:

- It consumes the `[Unreleased]` heading. Add a fresh `## [Unreleased] - TBD` section after
  every release, or the next bump has nothing to match.
- `sed` exits 0 when its pattern matches nothing, so the script's error checks never fire on a
  missed substitution. Confirm each file actually changed (`git diff`).
- The CHANGELOG date is the day the script ran, not necessarily the release date.
- The version regex is a bare `[0-9]+\.[0-9]+\.[0-9]+` applied to every line. Today only the
  version lines match, but if a dependency version or another dotted three-part number is ever
  added to one of those files, it will get rewritten too.
...
- `_version.py` — tracked in git, but rewritten by `setup.py` at install time and by
  `.release/changeVersionNumbers.sh`. Never edit it by hand; it is also excluded from pylint.
...
- Any user-facing change should get a one-line entry in `CHANGELOG.md`, under
  ...
  `CONTRIBUTING.md` requires this twice — it's not optional. If there is no `[Unreleased]`
  section (as happens right after a release), create one above the newest release. Never
  append to a section that has already been released.
...
- Source modules generally start with a docstring giving attribution/citation context (several
  ...
  when editing.
- `.gitattributes` normalizes line endings to LF (never write CRLF into source or scripts),
  except `*.bat` (e.g. `docs/make.bat`), which is checked out CRLF because `cmd.exe` needs it.
  It marks `.nc`/`.npy`/`.npz`/`.pdf`/images as binary, with `*.ipynb` diff-suppressed.

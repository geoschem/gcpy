...
def file_regrid(fin, fout, dim_format_in, dim_format_out, cs_res_out=0, ll_res_out='0x0', sg_params_in=[1.0, 170.0, -90.0], sg_params_out=[1.0, 170.0, -90.0]):
    ...
    if cs_res_in == cs_res_out and all([v1 == v2 for v1, v2 in zip(sg_params_in, sg_params_out)]):
        ...
        regridders = make_regridder_S2S(
            ...
            sf_in=sg_params_in[0], tlon_in=sg_params_in[1], tlat_in=sg_params_in[2],
            sf_out=sg_params_out[0], tlon_out=sg_params_out[1], tlat_out=sg_params_out[2]
        )
        ...
        ds_out = xr.concat([regridders[face](ds_in, keep_attrs=True) for face in range(6)], dim='nf')
        #flip vertical
    ...
    if dim_format_in != 'classic' and dim_format_out != 'classic':
        ...
        ds_out = reformat_dims(ds_out, format=dim_format_out, towards_common=False)
...
def rename_restart_variables(ds, towards_gchp=True):
    ...
    if towards_gchp:
        ...
        old_str = 'SPC'
        new_str = 'SpeciesRst'
    return ds.rename({name : name.replace(old_str, new_str, 1) for name in list(ds.data_vars) if name.startswith(old_str)})
...
def drop_and_rename_classic_vars(ds, towards_gchp=True):
    ...
    if towards_gchp:
        ...
        ds = ds.drop_vars(['P0', 'hyam', 'hybm', 'hyai', 'hybi', 'AREA', 'ilev',
                           'PS1DRY', 'PS1WET', 'TMPU1', 'SPHU1', 'StatePSC'], errors='ignore')
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ll_res_out',
                        metavar='RES',
                        type=str,
                        required=False,
    ...
    parser.add_argument('--dim_format_in',
                        metavar='WHICH',
                        type=str,
                        choices=['checkpoint', 'diagnostic', 'classic'],
                        required=True,

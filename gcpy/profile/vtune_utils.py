...
def vtune_how_is_report_grouped(
    ...
    """
    if "Source Line" in dframe.columns:
        return "by_line"
...
def vtune_list_all_hotspots(
    ...
    if "by_line" in vtune_how_is_report_grouped(dframe):
    ...
    if "by_function" in vtune_how_is_report_grouped(dframe):
...
def vtune_find_hotspot(
    ...
    Args
    dframe       : pd.DataFrame : DataFrame object w/ hotspot information
    ...
    index = dframe[column] == hotspot_name
...
def vtune_compare_a_hotspot_by_line(
    ...
    Args
    ref_data        : pd.DataFrame : Hotspot report from the Ref model
    ref_label       : str          : Label for the Ref model
    dev_data        : pd.DataFrame : Hotspot report from the Dev model
    ...
    def get_hotspot(data, hotspot_name, line_number):
        """
        Returns info about a hotspot from a report (grouped by function).

        Args
        data         : pd.DataFrame : Hotspot data
        ...
        data = data[["Source File", "Source Line", "CPU Time [s]"]]
        data = vtune_find_hotspot(data, "Source File", hotspot_name)
        print(data)
        data = vtune_find_hotspot(data, "Source Line", line_number)
        print(data)
        
        # Return as list: [Rank, Hotspot Name, Source Line, CPU Time]
        values = data.iloc[0].tolist()
        return [data.index[0], values[0], values[1], values[2]]

    # Return hotspot info as Rank, Name, Line, CPU Time
    ref_hotspot = get_hotspot(ref_data, hotspot_name, ref_line_number)
    dev_hotspot = get_hotspot(dev_data, hotspot_name, dev_line_number)
...
def vtune_compare_a_hotspot_by_function(
    ...
    def get_hotspot(data, hotspot_name):
        """
        Returns info about a hotspot from a report (grouped by function).
        ...
        """
        # Return the hotspot as a DataFrame with one row
        data = data[["Function", "CPU Time [s]"]]
        data = vtune_find_hotspot(data, "Function", hotspot_name)

        # Return as list: [Rank, Hotspot Name, CPU Time]
        values = data.iloc[0].tolist()
        return [data.index[0], values[0], values[1]]

    # Return hotspot info as Rank, Name, Line, CPU Time
    ref_hotspot = get_hotspot(ref_data, hotspot_name)
    dev_hotspot = get_hotspot(dev_data, hotspot_name)
...
def vtune_compare_a_hotspot(
    ...
    ref_data = vtune_read_hotspots_csv(ref_file, delimiter=delimiter)
    ref_grouped = vtune_how_is_report_grouped(ref_data)

    # Read Dev data and determine how it is grouped
    dev_data = vtune_read_hotspots_csv(dev_file, delimiter=delimiter)
    dev_grouped = vtune_how_is_report_grouped(dev_data)

...
ESMF_RegridWeightGen, a template file, and optional stretched-grid
parameters, to produce a regridded GCHP restart file.
...
def is_gchp_restart_file(dataset):
    ...
    -------
    is_restart : bool
        True if `dataset` represents a GCHP restart file.
    """
...
def open_dataset(file_or_url, chunk_size=8192):
    ...
    ----------
    file_or_url : str
        A file path on the local system or URL.
    chunk_size : int
        Size of chunks to stream from remote dataset to the local system.

    Returns
    -------
    dataset : xarray.Dataset
        An xarray dataset containing the restart file data.
    """
    global TEMP_FILES
...
def rename_variables(dataset, to_gchp=True):
    ...
    ----------
    dataset : xarray.Dataset
        The dataset to have its variables renamed.
    to_gchp : bool
        True if converting to GCHP naming convention, False if converting
        to GC-Classic.
    """
    to_gchp_re_sub = [
...
def reverse_lev(dataset, to_gchp):
    ...
    ----------
    dataset : xarray.Dataset
        The dataset to have its level index reversed.
    to_gchp : bool
        True if we are saving out a GCHP restart file.

    Returns
    -------
    dataset : xarray.Dataset
        The input dataset with a reversed level index.
    """
...
def drop_variables(dataset, output_template):
    ...
    ----------
    dataset : xarray.Dataset
        The dataset from which to drop variables.
    output_template : xarray.Dataset
        The template from which to determine variables to drop.

    Returns
    -------
    dataset : xarray.Dataset
        The input dataset with variables dropped.
    """
...
def regrid(dataset, output_template, weights_file):
    ...
    output_template : xarray.Dataset
        The template file for the regridded output.
    weights_file : xarray.Dataset
        The precalculated regridding weights, generated by
        ESMF_RegridWeightGen.
...
def update_encoding(dataset):
    ...
    ----------
    dataset : xarray.Dataset
        The dataset to have its variable encoding checked and updated.

    Returns
    -------
    dataset : xarray.Dataset
        The input dataset with float32 variable encoding applied.
    """
...
def regrid_restart_file(
    ...
    """
    Perform an end-to-end regridding from reading input gridded data and
    regridding weights to writing out the regridded data.
    
    Parameters
    ----------
    input_restart : str
        The path to the restart file that will be regridded.
    regrid_weights : str
        The path to the regridding weights, generated by
        ESMF_RegridWeightGen.
    output_restart_template : str
        The path to the regridding output template file.
    stretch_factor : float, optional
        An optional stretch factor, for use with stretched-regridding.
    target_lat : float, optional
        An optional target latitude, for use with stretched-regridding.
    target_lon : float, optional
        An optional target longitude, for use with stretched-regridding.
    """
